		...
		for(var y = 0; y < board[0].length; y++){
			...
			if(board[x][y]!=0){
				...
				if(board[x][y].value<16){
					...
					ctx.fillText(board[x][y].value, board[x][y].position.x * 100 + 10, board[x][y].position.y * 100 + 60);
				}
			}
		}
	}
			...
			for(var y = 0; y < board[0].length; y++){
				if(board[x][y]!=0){
					for(var i = 0; i < 3; i++){
						...
						else if(x-i-1>=0 && board[x-i-1][y].value==board[x-i][y].value && board[x-i-1][y].canSum == true){
							board[x-i-1][y].value *= 2;
							score+=board[x-i-1][y].value;
							counter++;
							board[x-i][y] = 0;
							board[x-i-1][y].canSum = false;
							break;
			...
			for(var y = 0; y < board[0].length; y++){
				if(board[x][y]!=0){
					for(var i = 0; i < 3; i++){
						...
						else if(x+i+1<=3 && board[x+i+1][y].value==board[x+i][y].value && board[x+i+1][y].canSum == true){
							board[x+i+1][y].value *= 2;
							score+=board[x+i+1][y].value;
							counter++;
							board[x+i][y] = 0;
			...
			for(var y = 0; y < board[0].length; y++){
				if(board[x][y]!=0){
					for(var i = 0; i < 3; i++){
						...
						else if(y-i-1>=0 && board[x][y-i-1].value==board[x][y-i].value && board[x][y-i-1].canSum == true){
							board[x][y-i-1].value *= 2;
							score+=board[x][y-i-1].value;
							counter++;
							board[x][y-i] = 0;
							board[x][y-i-1].canSum = false;
							break;
						}
					}
			...
			for(var y = board[0].length-1; y >= 0; y--){
				if(board[x][y]!=0){
					for(var i = 0; i < 3; i++){
						...
						else if(y+i+1<=3 && board[x][y+i+1].value==board[x][y+i].value && board[x][y+i+1].canSum == true){
							board[x][y+i+1].value *= 2;
							score+=board[x][y+i+1].value;
							counter++;
							board[x][y+i] = 0;
...
function verifyLoss(){
	...
	if(number_of_pieces==16){
		...
		if(!canJoin){
				document.getElementById('score').innerHTML = 'You lost! Final score: '+score+'<br>Press spacebar to restart';
		}
	}
...
function baguncinha(){
	for(var x = 0; x < board.length; x++){
		for(var y = 0; y < board[0].length; y++){
			...
			board[x][y].value=Math.floor(Math.random() * 666);
		}
	}
}
